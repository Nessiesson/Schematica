...
public class Paster {
    ...
    public static boolean setBlockAt(final IBlockState blockState, final BlockPos pos, final World world, final boolean canPlaceBlockAt) {
        Block block = blockState.getBlock();
        boolean successfull = false;
        if(!canPlaceBlockAt || block.canPlaceBlockAt(world, pos)) {
            if (pos.getY() > 0 && pos.getY() < 256 && world.isBlockLoaded(pos) && !(block instanceof BlockPistonExtension)) {
